 ...
 *      (via the Overpass API), filtered to labeled, single, non-runway-
 ...
 *
 * IMPORTANT — data quality filters (v6):
 *   - Both OSM nodes AND ways tagged aeroway=parking_position are used.
 *   - Only elements with an explicit `ref`/`name` tag are kept.
 *   - Runway sanity check: any candidate within RUNWAY_EXCLUSION_METERS of
 *     a real runway centerline (pulled from OSM aeroway=runway geometry) is
 *     dropped — this removes physically-impossible mistagged points.
 *   - NEW: multi-bay/merged refs are dropped. Some OSM elements are tagged
 *     with combined refs like "17A;17B", "18;19", or "96A, 96B" — these
 *     represent ambiguous double-bay markings, not one specific numbered
 *     stand, and cross-checking against real Airservices Australia apron
 *     charts (YSSY) confirms these aren't valid single-bay identifiers.
 *     Any ref containing ";", "," or whitespace-separated multiple tokens
 *     is now excluded rather than shown as a fake single gate.
 *   - NEW: known officially-deleted bay numbers can be excluded via
 *     DELETED_BAYS (per-airport, keyed loosely by ref) — e.g. YSSY bays
 *     104 and 107 were deleted per the 09 JUL 2026 Airservices Australia
 *     apron chart but can linger in community-maintained OSM data.
 *
 * IMPORTANT — occupancy classification:
 *   A gate is marked "occupied" as soon as a live VATSIM pilot's position is
 *   found within CONFIG.matchRadiusMeters of it. This is still an inference,
 *   never a guarantee — VATSIM has no gate field, and OSM stand coordinates
 *   are community-maintained and vary in precision/completeness by airport.
 *
 * IMPORTANT — serverless note:
 *   The in-memory `pollHistory` Map only persists within a single warm
 *   function instance and only across requests landing on that instance.
 *
 * IMPORTANT — Overpass reliability note:
 *   overpass-api.de is a free, shared, public server; retries with backoff
 *   are built in for 429/502/503/504.
 */
...
function distanceToNearestRunway(point, runwayLines) {
  let min = Infinity;
  for (const line of runwayLines) {
...
async function fetchAirportGates(airportLat, airportLon) {
  ...
  const [gateData, runwayData] = await Promise.all([
    overpassQuery(gateQuery),
    overpassQuery(runwayQuery).catch(() => ({ elements: [] })),
  ]);

  const runwayLines = (runwayData.elements || [])
  ...
  const deduped = dedupeGatesByCode(rawGates);

  if (runwayLines.length === 0) return deduped;

  return deduped.filter(
    (gate) => distanceToNearestRunway(gate, runwayLines) >= CONFIG.runwayExclusionMeters
  );
}
