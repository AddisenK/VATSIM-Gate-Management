 ...
 *
 * fetchAirportGates(): single combined Overpass query using ONLY the
 * `geom` output modifier (not `center` combined with `geom`, which appears
 * to behave unreliably when a query mixes node and way element types in a
 * single request — it was silently returning zero elements). Way centers
 * are now computed locally by averaging their geometry points instead of
 * relying on Overpass's `center` modifier.
 */
