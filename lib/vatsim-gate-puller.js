 ...
 *   field, so this is always an inference, never a guarantee. The
 *   `confidence` field reports how many consecutive polls (within the same
 *   warm serverless instance) have confirmed that same aircraft/gate pairing,
 *   but does NOT gate whether a gate shows as occupied — a single confirmed
 *   sighting is enough to mark it occupied, since on serverless platforms
 *   multi-poll history rarely accumulates within one HTTP request lifetime.
 *
...
const CONFIG = {
  ...
  overpassMaxRetries: 3,     // retry attempts on 502/503/504 or network failure
  overpassRetryDelayMs: 1500,
  overpassTimeoutMs: 20000,  // abort a single attempt if it hangs this long
};
 ...
 * and returns gate-by-gate occupancy. A gate is "occupied" as soon as a live
 * pilot matches its position — confidence is reported alongside as extra
 * context, not as a filter on the status itself.
 */
async function pullAndOverlay(airportLat, airportLon, gateOverride = null) {
  ...
  // update rolling poll count per callsign (informational confidence signal only)
  ...
  const results = gates.map((gate) => {
    ...
    // A single confirmed live-position match is enough to mark the gate occupied.
    // Confidence still reported for transparency, but never blocks the status.
    return {
      gate: gate.code,
      lat: gate.lat,
      lon: gate.lon,
 ...
 * (a Node server, CLI, or worker) — not needed for a single serverless request
 * now that occupancy no longer requires multiple polls.
 */
