 ...
 *
 * fetchAirportGates(): a single combined Overpass query (gates + runways +
 * aprons/taxiways). Timeout raised to 15s (realistic for larger/busier
 * airports with lots of taxiway geometry) and in-request retries removed —
 * a failed attempt fails fast and relies on the client's own backoff retry
 * instead of hammering Overpass again within the same request, which was
 * contributing to intermittent 502s.
 *
 * matchOccupancy(): cheap, VATSIM-only, safe on every 15s refresh tick.
 * This was already 100% reliable — the errors were isolated to the gate
 * map lookup, not the live traffic check.
 */
