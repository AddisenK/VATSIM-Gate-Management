 ...
 *   - fetchAirportGates(): now issues ONE combined Overpass query (gates +
 *     runways + aprons/taxiways in a single HTTP round-trip) instead of 3
 *     separate requests. Vercel serverless functions have a hard execution
 *     time limit; 3 sequential/parallel HTTP calls to a slow public API
 *     (Overpass) could exceed it and get killed mid-request, which shows up
 *     client-side as "fetch failed." One combined query cuts network
 *     overhead roughly 3x and keeps everything inside a single timeout
 *     budget.
 *   - matchOccupancy(): unchanged — cheap, VATSIM-only, safe on every
 *     15s refresh tick.
 */
...
// EXPENSIVE (but now a SINGLE Overpass request instead of 3) — call this
// rarely (once per location/airport). The physical layout of an airport
// barely ever changes; cache the result client-side or via CDN headers.
