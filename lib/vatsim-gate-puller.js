 ...
 *
 * IMPORTANT — data quality filters (v3):
 *   - Only OSM NODES tagged aeroway=parking_position are used. OSM "way"
 *     elements with this tag are usually lead-in lines / taxi paths rather
 *     than the actual stand position, and their centroid can land in the
 *     middle of an apron or taxiway — this was causing stray, meaningless
 *     dots on the map. Ways are now excluded entirely.
 *   - Only nodes with an explicit `ref` (or `name`) tag are kept. Untagged
 *     nodes (previously falling back to "OSM-<id>") are usually unlabeled
 *     apron/taxi points, not real numbered gates, and are now dropped.
 *   - matchRadiusMeters reduced from 70m to 30m. 70m was wide enough that
 *     an aircraft stopped on a taxiway or ramp near a cluster of stands
 *     could incorrectly match a nearby gate. 30m still absorbs reasonable
...
const CONFIG = {
  matchRadiusMeters: 30,     // tightened from 70m to avoid taxiway/ramp false matches
  nearbyRadiusKm: 6,
  maxStaleMs: 30_000,
  minPollsForMedium: 2,
  minPollsForHigh: 3,
...
// Merge OSM nodes that share the same gate code into one representative point.
...
async function fetchAirportGates(airportLat, airportLon) {
  ...
  // Only NODES — way elements tagged parking_position are lead-in lines/taxi
  // paths, not stand positions, and their centroid is unreliable.
  const query = `
    [out:json][timeout:25];
    (
    ...
    try {
      ...
      const rawGates = data.elements
        ...
        .map((el) => {
          ...
          if (!ref) return null; // drop unlabeled nodes — not real numbered gates
          if (el.lat == null || el.lon == null) return null;
          return { code: ref, lat: el.lat, lon: el.lon, source: "osm" };
        })
        .filter(Boolean);
...
async function pullAndOverlay(airportLat, airportLon, gateOverride = null) {
  ...
  // For each aircraft, find its single nearest gate within range (if any),
  // rather than matching every gate within radius — prevents one aircraft
  // from lighting up multiple neighboring stands.
  const gateByAircraft = new Map(); // gate.code -> { pilot, distance }
  candidates.forEach((p) => {
    let nearest = null;
    ...
    if (nearest) gateByAircraft.set(nearest.code, { pilot: p, distance: nearestDist });
  });

  const results = gates.map((gate) => {
    const match = gateByAircraft.get(gate.code);
    if (!match) {
