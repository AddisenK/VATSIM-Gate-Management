 ...
 *      (via the Overpass API), filtered to labeled stands only,
 *      deduplicated by gate code, AND sanity-checked against runway
 *      geometry to drop physically impossible points.
 ...
 *
 * IMPORTANT — data quality filters (v5):
 *   - Both OSM nodes AND ways tagged aeroway=parking_position are used.
 *   - Only elements with an explicit `ref` (or `name`) tag are kept —
 *     unlabeled elements are dropped as likely taxi/lead-in artifacts.
 *   - NEW: runway sanity check. Some airports (observed at YSSY) have a
 *     handful of community-tagged parking_position points whose raw OSM
 *     coordinates fall directly on or immediately beside an active runway —
 *     physically impossible for a real aircraft stand. fetchAirportGates()
 *     now also pulls all aeroway=runway way geometries in the same bounding
 *     box and drops any candidate gate within RUNWAY_EXCLUSION_METERS of a
 *     runway centerline. This is a data-integrity filter, not a
 *     VATSIM/live-traffic filter — it runs once per gate-map fetch.
 *   - matchRadiusMeters stays at 30m; each aircraft still matches only its
 *     single nearest gate within range.
 *
